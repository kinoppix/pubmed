...
# for testing
# _URLs = [f"https://ftp.ncbi.nlm.nih.gov/pubmed/baseline/pubmed23n{i:04d}.xml.gz" for i in range(1166, 1167)]
# modify README.md
#    num_bytes: 16366464
#    num_examples: 10711
...
class Pubmed(datasets.GeneratorBasedBuilder):
    ...
    def xml_to_dictionnary(self, parentElement):
        ...
        if parentElement.tag in {"ArticleTitle"}:
            # XXX
        ...
        if parentElement.tag in {"AbstractText"}:
            # XXX
            # Very special case, it will contain html leading to having very odd structure
            prefix = parentElement.attrib['Label'] + ":\n" if 'Label' in parentElement.attrib else ''
            element_list = list(parentElement.iter())
            inner_string = ''.join([(element.text or "") + ("" if element.tag == "AbstractText" else (element.tail or "")) for element in element_list]) 
            return {parentElement.tag: prefix + inner_string}

        for child in list(parentElement):
            child.text = child.text if (child.text is not None) else " "
            key = child.tag
            if len(child) == 0:
                # add abstract label to abstract text
                if key == "AbstractText":
                    prefix = child.attrib['Label'] + ":\n" if 'Label' in child.attrib else ''
                else:
                    prefix = ""
                value = prefix + child.text.strip()
            else:
    ...
    def _info(self):
        ...
        Reference = {
            ...
            # "ArticleIdList": {"ArticleId": datasets.Sequence(datasets.Value("int32"))},
        ...
        Article = {
            # 'Journal': Journal,
            "Abstract": {"AbstractText": datasets.Sequence(datasets.Value("string"))},
